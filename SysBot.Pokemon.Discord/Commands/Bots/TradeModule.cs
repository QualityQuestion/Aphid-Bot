...
namespace SysBot.Pokemon.Discord
{
    ...
    public class TradeModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task TradeAsync([Summary("Trade Code")] int pseudorand, [Summary("Showdown Set")][Remainder] string content)
        {
            ...
            {
                var msg = $"Unable to parse ??d: value";
        ...
        public static void LedybotIdleTask(int ledybotdex, int level, int gender, string nickname, bool shininess)
        {
            ...
            if (epoch % 6 == 1)
            {
                nature = "\nModest Nature"; // 1/6 chance of Modest
                if(level >= 80)
                {
            ...
            {
                nature = "\nAdamant Nature"; // 1/6 chance of Adamant
                if (level >= 80)
                {
            ...
            {
                nature = "\nTimid Nature"; // 1/6 chance of Timid
                if (level >= 80)
                {
                    evs = "\nEVs: 252 SpA / 4 SpD / 252 Spe";
                }
            ...
            {
                nature = "\nQuiet Nature"; // 1/6 chance of Quiet
                if (level >= 80)
                {
            ...
            {
                nature = "\nJolly Nature"; // 1/6 chance of Jolly
                if (level >= 80)
                {
                    evs = "\nEVs: 252 Atk / 4 SpD / 252 Spe";
                }
            ...
            {
                nature = "\nBrave Nature"; // 1/6 chance of Brave
                if (level >= 80)
                {
            ...
            try
            {
                string content = "" + speciesstring + gendersymbol + evs + "\nLevel: " + level + shinyYes + nature;
                var set = new ShowdownSet(content);
                ...
                MoveSourceType typez = MoveSourceType.AllTutors;
                MoveSourceType tipez = MoveSourceType.AllMachines;
                if (pkm.Species != 800)
                {
                    pkm.Moves = MoveListSuggest.GetSuggestedCurrentMoves(la, MoveSourceType.ExternalSources);
                }
